...
RunBeat 最复杂的交互模块，提供波形可视化预览和首拍时间微调功能。

功能:
    1. 自动加载 audio_output/shifted_songs/ 中的变速后音频
    2. 使用 FirstBeatDetector 自动检测每首歌的首拍位置
    3. 波形可视化预览（前 15 秒），支持:
       - 鼠标拖拽红色首拍线 / 黄色播放头线
       - 鼠标拖拽平移波形视图
       - 滚轮缩放时间窗口（5s ~ 全时长）
       - 左键双击跳转播放位置 / 右键双击跳转首拍位置
       - 方向键微调首拍（±10ms）
    4. 音频播放: 通过 sounddevice.OutputStream 实时流式播放
       支持播放/暂停/停止/跳转/音量调节
    5. 精细调校面板: ±10ms / ±100ms 按钮 + 数值输入框
    6. 所有修改实时保存到 data/beat_alignments.json

类:
    BeatDetectThread  后台首拍检测线程
    AudioLoader       后台音频文件加载线程
    AudioPlayer       音频播放线程（sounddevice 回调模式）
    WaveformWidget     波形绘制组件（深色主题，mp3.py 风格）
    BeatAlignTab       标签页主控件

首拍对齐在混音中的作用:
    每首歌的首拍会与节拍器的重拍（每 4 拍 = 1 小节）对齐，
    确保所有歌曲和节拍器同步，跑步时节奏一致。
"""
